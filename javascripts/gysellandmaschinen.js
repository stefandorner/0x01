    ...
    a.fn.TopSlider = function (b) {
        ...
        return this.each(function () {
            var b = a(this), d = b.attr("id"), e = c.blogURL + "/feeds/posts/default/" + (0 == c.labelName.length ? '' : "-/" + c.labelName.replace(/\,(\s+)?/g, "/")) + "?max-results=" + c.maxItem + "&orderby=published&alt=json-in-script"; a.ajax({
                ...
                complete: function () { a("#" + d).bxSlider({pager: false, auto: true, nextSelector: '#bxSliderBanner-next',prevSelector: '#bxSliderBanner-prev',nextText: '<span class="fa fa-arrow-circle-right fa-3x"/>',prevText: '<span class="fa fa-arrow-circle-left fa-3x"/>'}); }
            })
        })
    }
}(jQuery);
    ...
    a.fn.MaschinenSlider = function (b) {
        ...
        return this.each(function () {
            var b = a(this), d = b.attr("id"), e = c.blogURL + "/feeds/posts/default/" + (0 == c.labelName.length ? '' : "-/" + c.labelName.replace(/\,(\s+)?/g, "/")) + "?max-results=" + c.maxItem + "&orderby=published&alt=json-in-script"; a.ajax({
                ...
                complete: function () { a("#" + d).bxSlider({pager: false, auto: true, nextSelector: '#bxSliderMaschinen-next',prevSelector: '#bxSliderMaschinen-prev',nextText: '<span class="fa fa-arrow-circle-right fa-3x"/>',prevText: '<span class="fa fa-arrow-circle-left fa-3x"/>'}); }
            })
        })
    }
}(jQuery);
